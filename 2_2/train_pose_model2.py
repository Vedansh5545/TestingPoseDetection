...
class MPIINF3DHPDataset(Dataset):
    def __init__(self, npz_path):
        ...
        # Ensure both are [28, 3] by padding z=0 to 2D
        if pose2d.shape[-1] == 2:
            zeros_z = np.zeros((pose2d.shape[0], 28, 1), dtype=np.float32)
            pose2d = np.concatenate([pose2d, zeros_z], axis=-1)
...
class COCO_MediaPipeDataset(Dataset):
    def __init__(self, npz_path):
        data = np.load(npz_path)
        pose2d_raw = data['pose2d'].astype(np.float32)

        self.pose2d_mean = pose2d_raw.mean(axis=(0, 1))
        self.pose2d_std = pose2d_raw.std(axis=(0, 1))

        pose2d_norm = (pose2d_raw - self.pose2d_mean) / (self.pose2d_std + 1e-6)
